...
const Flashcards: React.FC<FlashcardsProps> = ({ cards }) => {
  ...
  return (
    <div className="flex flex-col items-center space-y-8">
      <div className="relative w-96 h-64 perspective cursor-pointer" onClick={() => setFlipped(!flipped)}>
        <div className={`relative w-full h-full rounded-3xl shadow-2xl transition-transform duration-700 transform-style-preserve-3d ${flipped ? "rotate-y-180" : ""}`}>
          ...
          <div className="absolute w-full h-full bg-gradient-to-br from-gray-400 to-white rounded-3xl shadow-lg flex flex-col justify-center p-6 backface-hidden">
            ...
            <p className="mt-2 text-gray-700 text-center text-sm">Click để xem chi tiết</p>
      ...
      <div className="flex space-x-4">
        <button onClick={prevCard} className="px-6 py-2 bg-gray-300 rounded-lg hover:bg-gray-400 transition">
          Prev
        </button>
        <button onClick={nextCard} className="px-6 py-2 bg-gray-300 rounded-lg hover:bg-gray-400 transition">
          Next
        </button>
      </div>
    </div>
