...
interface SpeakingItem { // Để hiển thị context
  id: number;
  topic: string;
  speakingType: string;
}
...
const TeacherManageOverviewSpeakingQuestionsPage: React.FC = () => {
  ...
  const fetchSpeakingItemInfo = async () => {
    ...
    try {
      ...
      console.error("❌ Lỗi tải thông tin Speaking Item:", err);
      // Không cần set notif ở đây, lỗi chính là ở fetchQuestions
    }
  };
  ...
  const fetchQuestions = async () => {
    ...
    try {
      setLoading(true);
      // Sử dụng API GET /api/speaking-questions?speakingId={speakingId}
      const res = await axios.get(`${API_URL}/speaking-questions`, {
        headers: { Authorization: `Bearer ${token}` },
        params: { speakingId: speakingId }, // Truyền speakingId làm query parameter
      });
      setQuestions(res.data.result || res.data || []);
    } catch (err) {
      console.error("❌ Lỗi tải Speaking Questions:", err);
  ...
  // --- Fetch initial data ---
  useEffect(() => {
    fetchSpeakingItemInfo(); // Lấy thông tin item cha
    fetchQuestions();       // Lấy danh sách câu hỏi con
  }, [speakingId]);
  ...
  const handleSaveQuestion = async () => {
    ...
    Object.keys(payload).forEach(key => {
        if (payload[key] === '' && ['id', 'speakingId', 'questionRomaji', 'questionMeaning', 'answer', 'answerRomaji', 'answerMeaning'].includes(key)) {
            payload[key] = null; // Hoặc delete payload[key];
        }
    });

    if (editingQuestion) {
      payload.speakingId = Number(speakingId); // API PUT yêu cầu speakingId
      url = `${API_URL}/speaking-questions/${editingQuestion.id}`;
      method = "put";
    } else {
      delete payload.id;
      payload.speakingId = Number(speakingId); // Thêm speakingId để liên kết
  ...
  const renderSpeakingQuestionModalForm = () => (
     <div className="flex flex-col gap-4">
        <textarea placeholder="Câu hỏi (*)" value={questionFormData.question || ""} onChange={(e) => handleQuestionChange("question", e.target.value)} className="border p-3 rounded-lg min-h-[80px]" required />
        <input type="text" placeholder="Romaji câu hỏi" value={questionFormData.questionRomaji || ""} onChange={(e) => handleQuestionChange("questionRomaji", e.target.value)} className="border p-3 rounded-lg"/>
        <input type="text" placeholder="Nghĩa câu hỏi" value={questionFormData.questionMeaning || ""} onChange={(e) => handleQuestionChange("questionMeaning", e.target.value)} className="border p-3 rounded-lg"/>
        <textarea placeholder="Câu trả lời gợi ý" value={questionFormData.answer || ""} onChange={(e) => handleQuestionChange("answer", e.target.value)} className="border p-3 rounded-lg min-h-[80px]" />
        <input type="text" placeholder="Romaji trả lời" value={questionFormData.answerRomaji || ""} onChange={(e) => handleQuestionChange("answerRomaji", e.target.value)} className="border p-3 rounded-lg"/>
        <input type="text" placeholder="Nghĩa trả lời" value={questionFormData.answerMeaning || ""} onChange={(e) => handleQuestionChange("answerMeaning", e.target.value)} className="border p-3 rounded-lg"/>
    </div>
  );
                ...
                {questions.map((q) => (
                  <motion.tr key={q.id} variants={fadeIn} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm text-gray-800 max-w-md">
                      ...
                      {q.questionRomaji && <div className="text-xs text-gray-500 italic mt-1">{q.questionRomaji}</div>}
                      {q.questionMeaning && <div className="text-xs text-gray-500 mt-1">({q.questionMeaning})</div>}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600 max-w-md">
                      <div>{q.answer || "-"}</div>
                       {q.answerRomaji && <div className="text-xs text-gray-500 italic mt-1">{q.answerRomaji}</div>}
                       {q.answerMeaning && <div className="text-xs text-gray-500 mt-1">({q.answerMeaning})</div>}
                    </td>
...
export default TeacherManageOverviewSpeakingQuestionsPage;